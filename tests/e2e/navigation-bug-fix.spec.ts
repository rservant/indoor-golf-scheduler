...
test.describe('Navigation Bug Fix Test', () => {
  test('should reproduce and verify the navigation bug fix', async ({ page }) => {
    ...
    let activeSeasonText = await activeSeasonDisplay.textContent();
    console.log(`Current active season display: ${activeSeasonText}`);

    if (!activeSeasonText || activeSeasonText.includes('No Active Season')) {
      ...
      activeSeasonText = await activeSeasonDisplay.textContent();
    ...
    if (inactiveCount > 0) {
      ...
      const newActiveSeasonText = await activeSeasonDisplay.textContent();
