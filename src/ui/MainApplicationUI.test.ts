...
jest.mock('./PlayerStatsDashboardUI', () => {
  return {
    PlayerStatsDashboardUI: jest.fn().mockImplementation(() => ({
      initialize: jest.fn().mockResolvedValue(undefined),
...
jest.mock('../services/PlayerStatsService', () => {
  return {
    PlayerStatsService: jest.fn().mockImplementation(() => ({
      ...
      getSeasonLeaderboard: jest.fn().mockResolvedValue([])
    }))
  };
});
...
describe('MainApplicationUI Navigation Bug Tests', () => {
  ...
  describe('Navigation Bug Reproduction', () => {
    test('should maintain clickable navigation after season switch', async () => {
      ...
      expect(navButtons).toHaveLength(6); // seasons, players, availability, schedule, stats, import-export
